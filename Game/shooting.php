 ...
 if(isset($_POST['resolve'])){
    ...
    $damage = array();
	    ...
	    for($i = 0; $i<count($dicesResult); $i++){
        	...
        	if(($target[$i] != '0')||($dicesResult[$i]>0)){

        		//doesn't work if unit is target of multiple sources because same ts but different rend and damage ==> need n-m table

        		$sql = "UPDATE unit_in_battle SET In_Combat_With = ".$source[$i].", Save_Request = ".$dicesResult[$i].", Save_Result = ".$rend[$i].", Damage = ".$damage[$i].", Shot = 1 WHERE FK_Battle = ".$battleID." AND FK_Unit = ".$target[$i];
        		mysql_query($sql) or die ("Could not update unit_in_Battle with stuff and things");
  ...
  <div class="content-wrapper">
    <div class="container-fluid">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Shooting</li>
      </ol>
      <div class="row">
        <div class="col-12">
          <h1>Shooting Phase - Round: <?php echo $round; ?></h1>
...
<form method="POST">
        ...
        ?>
			...
			<h3 for="ranBoxes">You have no shooting units available</h3>
 ...
 <h3 for="ranBoxes">Select shooting Units:</h3>
        ...
        while($row = mysql_fetch_array($resultSources)){
          ...
          while($datas = mysql_fetch_array($result)){
            ...
            <option value=<?php echo "'".$datas['TargetID']."'"; ?>><?php echo $datas['Target'] ?></option>
        ...
        <button type="submit" class="btn btn-primary btn-block" name="resolve" onmousedown="dice.play()" <?php if($succeded){ echo "disabled";} ?>> Deliver Them Your Punishment! </button>
